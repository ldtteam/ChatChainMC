...
public class ReplacementUtils
{
    ...
    public static String getFormat(final Group group, final Client client, final ClientUser clientUser, final FormatAction action)
    {
        final List<String> format = new ArrayList<>();

        if (ChatChainMC.instance.getMainConfig().getAdvancedFormatting())
        {
            format.addAll(ChatChainMC.instance.getAdvancedFormattingConfig().getOverride(group, client, clientUser, action));
        }
        else
        {
        ...
        for (final String formatSection : format)
        {
            ...
            while (motherMatcher.find())
            {
                ...
                final String replacementString = getReplacementForSection(group, client, clientUser, motherString);
    ...
    private static String getReplacementForSection(final Group group, final Client client, final ClientUser clientUser, final String formatsString)
    {
        ...
        while (childMatcher.find())
        {
            ...
            if (!matchString.equals(""))
            {
                ...
                final List<ClientRank> clientRanks = clientUser.getClientRanks();
                ...
                returnString = ClientUserReplacements.GetReplacementObject(clientUser, matchString);
                if (returnString != null)
                    return returnString;
            }
        }
