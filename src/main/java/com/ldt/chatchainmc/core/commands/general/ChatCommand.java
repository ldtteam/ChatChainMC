    ...
    public void execute(final MinecraftServer server, final ICommandSender sender, final String[] args) throws CommandException
    {
        if (args.length >= 1 && sender instanceof EntityPlayer)
        {
            ...
            if (channelStorage != null)
            {
                ...
                {
                    if ((ChatChainMC.instance.getMainConfig().createdChannels.contains(channel) || (channel.equalsIgnoreCase(StaticAPIChannels.MAIN) || channel.equalsIgnoreCase(StaticAPIChannels.STAFF))) && !channelStorage.getTalkingChannel().equalsIgnoreCase(channel))
                    {
